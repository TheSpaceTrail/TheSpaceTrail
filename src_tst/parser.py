...
from . import title_seq

# Import storyline
storyline = json.load(open("./src_tst/storyline.json", "r", encoding="utf-8"))
    ...
    while True:
        ...
        if sequence[idx][0] == "!":
            ...
            if split_sequence[0] == "!modify": # checks for what player value it's modifying then the amount to modify by then the operation for modification
                if not split_sequence[1] in player.keys(): player[split_sequence[1]] = 0
...
def execute_parse(console: rich.Console):
    ...
    state = run_sequence(storyline["init"], tstt)
